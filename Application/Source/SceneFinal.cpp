...
void SceneFinal::Init()
{
	...
	meshList[GEO_BACKGROUND] = MeshBuilder::GenerateQuad("Diag", Color(1, 1, 1), 1, 1.5, 1);
	meshList[GEO_BACKGROUND]->textureID = LoadTGA("Image//phoneUI//outside box.tga");
...
void SceneFinal::TransferGameInfo(Game* game)
{
...
void SceneFinal::Update(double dt)
{
	...
	timer2 += static_cast<float>(dt);
	if (((mouseToggle == true && Application::isMouseButtonPressed(0)) || timer2 >= 1) && !isBusy) {
		mouseToggle = false;
		timer2 = 0;
		step++;
	}
...
void SceneFinal::Render()
{
	...
	{
		RenderImageOnScreen(meshList[GEO_BACKGROUND], Color(1, 1, 1), 90, 60, 40, 30);
	}

	if (step >= 1) {
		RenderTextOnScreen(meshList[GEO_TEXT], "Final Grade:", Color(0.6f, 0.3f, 0.3f), 4, 10, 48);
	}
	if (step >= 2) {
		switch (grade)
		{
		case 1:
			RenderTextOnScreen(meshList[GEO_TEXT], "A", Color(0.3f, 0.9f, 0.3f), 20, 15, 30);
			break;
		case 2:
			RenderTextOnScreen(meshList[GEO_TEXT], "B", Color(0.4f, 0.8f, 0.3f), 20, 15, 30);
			break;
		case 3:
			RenderTextOnScreen(meshList[GEO_TEXT], "C", Color(0.5f, 0.7f, 0.3f), 20, 15, 30);
			break;
		case 4:
			RenderTextOnScreen(meshList[GEO_TEXT], "D", Color(0.6f, 0.6f, 0.3f), 20, 15, 30);
			break;
		case 5:
			RenderTextOnScreen(meshList[GEO_TEXT], "E", Color(0.7f, 0.5f, 0.3f), 20, 15, 30);
			break;
		case 8:
			RenderTextOnScreen(meshList[GEO_TEXT], "F", Color(0.9f, 0.3f, 0.3f), 20, 15, 30);
			break;
		default:
			break;
		}
	...
	if (step >= 3) {
		RenderTextOnScreen(meshList[GEO_TEXT], "Amount lost to", Color(0.6f, 0.3f, 0.3f), 4, 35, 50);
		RenderTextOnScreen(meshList[GEO_TEXT], "Phishing:", Color(0.6f, 0.3f, 0.3f), 4, 35, 48);
	}
	if (step >= 4) {
		RenderTextOnScreen(meshList[GEO_TEXT], "$" + std::to_string(int(Player->moneyLostToPhishing)) + ".00", Color(0.0f, 0.9f, 0.0f), 4, 40, 45);
	}
	if (step >= 5) {
		RenderTextOnScreen(meshList[GEO_TEXT], "Amount lost to", Color(0.6f, 0.3f, 0.3f), 4, 35, 40);
		RenderTextOnScreen(meshList[GEO_TEXT], "market scams:", Color(0.6f, 0.3f, 0.3f), 4, 35, 38);
	}
	if (step >= 6) {
		RenderTextOnScreen(meshList[GEO_TEXT], "$" + std::to_string(int(Player->moneyLostToMarket)) + ".00", Color(0.0f, 0.9f, 0.0f), 4, 40, 35);
	}
	if (step >= 7) {
		RenderTextOnScreen(meshList[GEO_TEXT], "Amount lost to", Color(0.6f, 0.3f, 0.3f), 4, 35, 30);
		RenderTextOnScreen(meshList[GEO_TEXT], "charity scams:", Color(0.6f, 0.3f, 0.3f), 4, 35, 28);
	}
	if (step >= 8) {
		RenderTextOnScreen(meshList[GEO_TEXT], "$" + std::to_string(int(Player->moneyLostToCharity)) + ".00", Color(0.0f, 0.9f, 0.0f), 4, 40, 25);
	}
	if (step >= 9) {
		RenderTextOnScreen(meshList[GEO_TEXT], "Amount lost to", Color(0.6f, 0.3f, 0.3f), 4, 35, 20);
		RenderTextOnScreen(meshList[GEO_TEXT], "spam ads:", Color(0.6f, 0.3f, 0.3f), 4, 35, 18);
	}
	if (step >= 10) {
		RenderTextOnScreen(meshList[GEO_TEXT], "$" + std::to_string(int(Player->moneyLostToAds)) + ".00", Color(0.0f, 0.9f, 0.0f), 4, 40, 14);
	}
	RenderTextOnScreen(meshList[GEO_TEXT], "Scamulation scam evaluation report purr", Color(0.6f, 0.3f, 0.3f), 2, 55, 1);
	if (step >= 11) {
		RenderTextOnScreen(meshList[GEO_TEXT], "Click anywhere to continue", Color(0.6f, 0.3f, 0.3f), 4, 20, 5);
	}
...
void SceneFinal::Reset()
{
}

void SceneFinal::UseScene()
{
	glBindVertexArray(m_vertexArrayID);
	glUseProgram(m_programID);
...
void SceneFinal::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SceneFinal::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y, float xRotation, float zRotation)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
	glDisable(GL_DEPTH_TEST); //uncomment for RenderTextOnScreen
...
std::vector<float> SceneFinal::getNumberValues(std::string filename)
{
...
void SceneFinal::RenderImageOnScreen(Mesh* mesh, Color color, float sizex, float sizey, float x, float y, float xRotation, float zRotation)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
	glDisable(GL_DEPTH_TEST); //uncomment for RenderTextOnScreen
