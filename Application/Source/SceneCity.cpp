...
void SceneCity::Init()
{
	...
	//light parameters
	{
		...
		light[3].position.Set(-195, 10, -42);
		light[3].color.Set(1.f, 0.4f, 0.2);
		light[3].power = 10.0f;
		light[3].kC = 1.f;
		light[3].kL = 0.01f;
		light[3].kQ = 0.001f;
	...
	//city
	{
		...
		meshList[GEO_CHARITY]->textureID = LoadTGA("Image//skin_woman.tga");
...
void SceneCity::TransferGameInfo(Game* game)
{
	...
	camera.CAMERA_SPEED = game->CAM_SPEED;
...
void SceneCity::Update(double dt)
{
	...
	distanceFromCharacter1 = camera.position.Distance(Vector3(156, 15, 162));
	distanceFromCharacter2 = camera.position.Distance(Vector3(28, 15, 384));
	angleFromCharacter1 = Vector3(156, 15, 162).AngleDifference(camera.position);
	angleFromCharacter2 = Vector3(28, 15, 384).AngleDifference(camera.position);

	if (distanceFromCharacter1 < 50 && distanceFromCharacter2 > 40 && isDialogue == false && hasTalkedChar1 == false) {
		isDialogue = true;
		speech = Dialogue(1);
	}
	if (distanceFromCharacter2 < 50 && distanceFromCharacter1 > 40 && isDialogue == false && hasTalkedChar2 == false) {
		isDialogue = true;
		speech = Dialogue(6);
	}
	if (distanceFromCharacter2 < 50 && mouseToggle && Application::isMouseButtonPressed(0) && speech == D_CHAR_CHOICE) {
		mouseToggle = false;
		if (42 < xpos && xpos < 58 && 7 < ypos && ypos < 13) {
			speech = D_CHAR_SCAM_2;
			Player->addMoney(-3000);
			Player->moneyLostToCharity += 3000;
			Player->addSocialMeter(20);
		}
		else if (22 < xpos && xpos < 38 && 7 < ypos && ypos < 13) {
			speech = D_LEAVE_2;
		}
	}
	if (distanceFromCharacter1 < 50 && mouseToggle && Application::isMouseButtonPressed(0) && speech == D_CHAR_CHOICE) {
		mouseToggle = false;
		if (42 < xpos && xpos < 58 && 7 < ypos && ypos < 13) {
			speech = D_CHAR_SCAM_1;
			Player->addSocialMeter(30);
		}
		else if (22 < xpos && xpos < 38 && 7 < ypos && ypos < 13) {
			speech = D_LEAVE;
			Player->addSocialMeter(-20);
	...
	if (distanceFromCharacter1 < 50 && mouseToggle && Application::isMouseButtonPressed(0) && isDialogue == true) {
		mouseToggle = false;
		if (speech == 5) {
			speech = D_CHAR_CHOICE;
		}
		else if (speech == D_CHAR_CHOICE) {
		}
		else if (speech == D_LEAVE) {
			hasTalkedChar1 = true;
			isDialogue = false;
		}
		else if (speech == D_CHAR_SCAM_1) {
			hasTalkedChar1 = true;
			isDialogue = false;
		}
		else {
			speech = Dialogue(speech + 1);
		}
	}
	if (distanceFromCharacter2 < 50 && mouseToggle && Application::isMouseButtonPressed(0) && isDialogue == true) {
		mouseToggle = false;
		if (speech == 11) {
			speech = D_CHAR_CHOICE;
		}
		...
		else if (speech == D_LEAVE_2) {
			hasTalkedChar2 = true;
			isDialogue = false;
		}
		else if (speech == D_CHAR_SCAM_2) {
			hasTalkedChar2 = true;
			isDialogue = false;
		}
		else {
	...
	if (distanceFromCharacter1 > 50 && distanceFromCharacter2 > 50) {
		NotificationTimer = 0;
		isDialogue = false;
	}

	switch (speech) {
	case D_CHAR_DIALOGUE_1:
		speakerName = "Isa";
		charTalk1 = "Hey! I'm Isa from humans against";
		charTalk2 = "dog cancer. Would you like to talk?";
		break;
	case D_CHAR_DIALOGUE_2:
		speakerName = "Isa";
		charTalk1 = "We are a private charity that is devoted";
		charTalk2 = "to improving the lives of dogs.";
		break;
	case D_CHAR_DIALOGUE_3:
		speakerName = "Isa";
		charTalk1 = "All we need you to do is to make a";
		charTalk2 = "donation on our website.";
		break;
	case D_CHAR_DIALOGUE_4:
		speakerName = "Isa";
		charTalk1 = "All proceeds will go to the great";
		charTalk2 = "dane cancer foundation.";
		break;
	case D_CHAR_DIALOGUE_5:
		speakerName = "Isa";
		charTalk1 = "Would you like to donate?";
		charTalk2 = "";
		break;
	case D_CHAR_DIALOGUE_6:
		speakerName = "Johnson";
		charTalk1 = "Hey! I'm John and i'm raising";
		charTalk2 = "awareness for earthquake help";
		break;
	case D_CHAR_DIALOGUE_7:
		speakerName = "Johnson";
		charTalk1 = "We are in desperate need for";
		charTalk2 = "funding and we need your help.";
		break;
	case D_CHAR_DIALOGUE_8:
		speakerName = "Johnson";
		charTalk1 = "Our charity aims to raise";
		charTalk2 = "10k at the end of the month";
		break;
	case D_CHAR_DIALOGUE_9:
		speakerName = "Johnson";
		charTalk1 = "Do note that we only accept money";
		charTalk2 = "through bank transfers!";
		break;
	case D_CHAR_DIALOGUE_10:
		speakerName = "Johnson";
		charTalk1 = "Would you like to donate?";
		charTalk2 = "";
		break;
	case D_LEAVE:
		speakerName = "Isa";
		charTalk1 = "Its fine. Bye then!";
		charTalk2 = "";
		break;
	case D_CHAR_SCAM_1:
		speakerName = "Isa";
		charTalk1 = "Thank you for your donation!";
		charTalk2 = "";
		break;
	case D_LEAVE_2:
		speakerName = "Johnson";
...
void SceneCity::Render()
{
	...
	Vector3 lightSpot3_camera = viewStack.Top() * light[3].spotDirection;
	glUniform3fv(m_parameters[U_LIGHT3_SPOTDIRECTION], 1, &lightSpot3_camera.x);
	...
	//render city
	{
		//characters
		{
			...
			RenderMesh(meshList[GEO_CHARITY], false);
			modelStack.PopMatrix();

			modelStack.PushMatrix();
			//scale, translate, rotate
			modelStack.Translate(156, 0, 162);
...
void SceneCity::Reset()
{
	...
	isDialogue = false;
	distanceFromCharacter1 = 0;
	distanceFromCharacter2 = 0;

	hasTalkedChar1 = false;
	hasTalkedChar2 = false;
...
void SceneCity::RenderSpeechBox()
{
	RenderImageOnScreen(meshList[GEO_NOTIF_BOX], Color(1, 1, 1), 60, 20, 40, 10);

	if (speech != D_CHAR_CHOICE) {
		RenderTextOnScreen(meshList[GEO_TEXT], speakerName + ":", Color(0.6f, 0.3f, 0.3f), 4, 14, 11);
		RenderTextOnScreen(meshList[GEO_TEXT], charTalk1, Color(0.6f, 0.3f, 0.3f), 3, 16, 8);
		RenderTextOnScreen(meshList[GEO_TEXT], charTalk2, Color(0.6f, 0.3f, 0.3f), 3, 16, 5);

		if (int(NotificationTimer) % 2 == 0) {
			RenderImageOnScreen(meshList[GEO_MOUSE], Color(1, 1, 1), 4, 4, 63, 7);
		}
		else {
			RenderImageOnScreen(meshList[GEO_MOUSE_RED], Color(1, 1, 1), 4, 4, 63, 7);
		}
	}
	else {
		if (42 < xpos && xpos < 58 && 7 < ypos && ypos < 13 && Application::isMouseButtonPressed(0)) {
			RenderImageOnScreen(meshList[GEO_BUTTON_PRESSED], Color(1, 1, 1), 15, 7, 50, 10);
			RenderTextOnScreen(meshList[GEO_TEXT], "yes !!", Color(1, 1, 1), 4, 45, 8);
		}
		else {
			RenderImageOnScreen(meshList[GEO_BUTTON_NOT_PRESSED], Color(1, 1, 1), 15, 7, 50, 10);
			RenderTextOnScreen(meshList[GEO_TEXT], "yes !!", Color(1, 1, 1), 4, 45, 9);
		}
		if (22 < xpos && xpos < 38 && 7 < ypos && ypos < 13 && Application::isMouseButtonPressed(0)) {
			RenderImageOnScreen(meshList[GEO_BUTTON_PRESSED], Color(1, 1, 1), 15, 7, 30, 10);
			RenderTextOnScreen(meshList[GEO_TEXT], "no !!", Color(1, 1, 1), 4, 25, 8);
		}
		else {
			RenderImageOnScreen(meshList[GEO_BUTTON_NOT_PRESSED], Color(1, 1, 1), 15, 7, 30, 10);
			RenderTextOnScreen(meshList[GEO_TEXT], "no !!", Color(1, 1, 1), 4, 25, 9);
		}
	}
}
