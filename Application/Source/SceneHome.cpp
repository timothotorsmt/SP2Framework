...
void SceneHome::Init()
{
	...
	//light parameters
	{
		...
		light[1].position.Set(0, 80, 0);
		light[1].color.Set(1.f, 0.8f, 0.6f);
		light[1].power = 5.f;
		light[1].kC = 1.f;
		light[1].kL = 0.01f;
		light[1].kQ = 0.001f;
		...
		light[2].position.Set(-21.7, 25, -8.1);
	...
	meshList[GEO_MOUSE] = MeshBuilder::GenerateQuad("watch", Color(1, 1, 1), 1, 1, 1);
	meshList[GEO_MOUSE]->textureID = LoadTGA("Image//Home//Mouse.tga");
	meshList[GEO_MOUSE_RED] = MeshBuilder::GenerateQuad("watch", Color(1, 1, 1), 1, 1, 1);
	meshList[GEO_MOUSE_RED]->textureID = LoadTGA("Image//Home//MouseRed.tga");
	meshList[GEO_KEYBOARD] = MeshBuilder::GenerateQuad("buttonnotpressed", Color(1, 1, 1), 1, 1, 1);
	meshList[GEO_KEYBOARD]->textureID = LoadTGA("Image//Home//WASD.tga");
...
void SceneHome::TransferGameInfo(Game* game)
{
	...
	game->CAM_SPEED = camera.CAMERA_SPEED;
...
void SceneHome::Update(double dt)
{
	if (tutorialStep >= 10) {
		...
		if (!hasReceivedTasks && NotificationTimer <= 0) {
			phone->addTaskToList("Buy HDMI cable");
			phone->addTaskToList("Go to work @ 10");
	...
	//boundaries
	{
		if (camera.position.x < -63) {

		}
		if (camera.position.x > 60) {

		}
		if (camera.position.z < -63) {

		}
		if (camera.position.z > 96) {
		}
	}
	...
	if (tutorialStep == 0) {
		...
		NotificationTimer += static_cast<float>(dt) * 2;
		if (mouseToggle && Application::isMouseButtonPressed(0)) {
			mouseToggle = false;
			camera.CAMERA_SPEED = 120;
			tutorialStep = 1;
			NotificationTimer = 0;
		}
	}
	...
	if (tutorialStep == 1) {
		...
		NotificationTimer += static_cast<float>(dt) * 3;
		if (mouseToggle && Application::isMouseButtonPressed(0)) {
			mouseToggle = false;
			tutorialStep = 2;
			NotificationTimer = 0;
	...
	if (tutorialStep == 2) {
		if (Application::isMouseButtonPressed(0) && 11 <= xpos && xpos <= 51 && ypos < 30) {
			camera.canLookAround = false;
			camera.CAMERA_SPEED = (xpos - 11) * 7.5 + 1;
		}
		else {
			camera.canLookAround = true;
		}
		if (55 < xpos && xpos < 69 && 11 < ypos && ypos < 17 && Application::isMouseButtonPressed(0) && mouseToggle) {
			mouseToggle = false;
			tutorialStep = 3;
		}
	}
	...
	if (tutorialStep == 4) {
		float distance = camera.position.Distance(Vector3(16, camera.position.y, 30));
		if (distance < 5) {
			clock->newDay();
			tutorialStep = 5;
		}
	...
	if (tutorialStep == 6) {
		if (mouseToggle && Application::isMouseButtonPressed(0)) {
			if ((65 > xpos || xpos > 75) && (45 > ypos || ypos > 55) && !isPhoneOpen) {
				mouseToggle = false;
				tutorialStep = 7;
			}
		}
	...
	if (tutorialStep >= 5) {
		...
		//go to city scene
		if (Application::IsKeyPressed('Z') && Application::IsKeyPressed('X')) {
			goNextScene = true;
		}
		...
		if (tutorialStep >= 6) {
			...
			if (!isPhoneOpen) {
				...
				case SceneHome::P_MONEY:
					break;
				case SceneHome::P_QUIT:
					if (52 < xpos && xpos < 68 && 19 < ypos && ypos < 27 && Application::isMouseButtonPressed(0) && mouseToggle) {
						goNextScene2 = true;
					}
					break;
...
{
	camera.Reset();
	money = int(DEFAULTMONEY); socialMeter = int(DEFAULTSOCIALMETER);
	...
	//tasks !!
	hasReceivedTasks = false;
	notifs = Notification::N_OFF;

	//phone ??
	phoneState = P_HOME;
	chatCount = 0;
	isPhoneOpen = false;
	for (bool x : hoverAnimation) {
		x = false;
	}
	NotificationTimer = 3;
	//task list
...
void SceneHome::Render()
{
	...
	//door light
	Position lightPosition1_cameraspace = viewStack.Top() * light[0].position;
	glUniform3fv(m_parameters[U_LIGHT0_POSITION], 1, &lightPosition1_cameraspace.x);
	...
	if (tutorialStep == 0) {
		RenderImageOnScreen(meshList[GEO_NOTIF_BOX], Color(1, 1, 1), 65, 45, 40, 30);
		RenderTextOnScreen(meshList[GEO_TEXT], "Welcome to the SCAMULATION!", Color(0.8f, 0.3f, 0.3f), 4, 14, 40);
		RenderTextOnScreen(meshList[GEO_TEXT], "this game is an educational simulation", Color(0.6f, 0.3f, 0.3f), 2.5f, 13, 36);
		RenderTextOnScreen(meshList[GEO_TEXT], "to teach people about avoiding everyday scams.", Color(0.6f, 0.3f, 0.3f), 2.5f, 13, 33);
		RenderTextOnScreen(meshList[GEO_TEXT], "firstly, let me show you the ropes!", Color(0.6f, 0.3f, 0.3f), 2.5f, 13, 30);
		RenderTextOnScreen(meshList[GEO_TEXT], "Click with the left mouse button to interact", Color(0.6f, 0.3f, 0.3f), 3, 13, 26);
		if (int(NotificationTimer) % 2 == 0) {
			RenderImageOnScreen(meshList[GEO_MOUSE_RED], Color(1, 1, 1), 10, 10, 40, 20);
		}
		else {
			RenderImageOnScreen(meshList[GEO_MOUSE], Color(1, 1, 1), 10, 10, 40, 20);
		}
	}
	if (tutorialStep == 1) {
		RenderImageOnScreen(meshList[GEO_NOTIF_BOX], Color(1, 1, 1), 40, 40, 40, 30);
		RenderTextOnScreen(meshList[GEO_TEXT], "Move the mouse", Color(0.6f, 0.3f, 0.3f), 3, 30, 39);
		RenderTextOnScreen(meshList[GEO_TEXT], "to look around", Color(0.6f, 0.3f, 0.3f), 3, 30, 36);
		RenderImageOnScreen(meshList[GEO_MOUSE], Color(1, 1, 1), 10, 10, 40 + sin(NotificationTimer) * 3, 26 + cos(NotificationTimer) * 3);
	}
	if (tutorialStep == 2) {
		RenderImageOnScreen(meshList[GEO_NOTIF_BOX], Color(1, 1, 1), 66, 20, 40, 15);
		RenderTextOnScreen(meshList[GEO_TEXT], "Set the camera speed to your liking", Color(0.6f, 0.3f, 0.3f), 3, 13, 18);
		RenderImageOnScreen(meshList[GEO_SOCIALSCORE_BAR], Color(1, 1, 1), (camera.CAMERA_SPEED) / 7.5, 3, 11 + (camera.CAMERA_SPEED) / 15, 14);
		if (55 < xpos && xpos < 69 && 11 < ypos && ypos < 17 && Application::isMouseButtonPressed(0)) {
			RenderImageOnScreen(meshList[GEO_BUTTON_PRESSED], Color(1, 1, 1), 15, 5, 62, 14);
			RenderTextOnScreen(meshList[GEO_TEXT], "confirm", Color(1, 1, 1), 3, 58, 12);
		}
		else {
			RenderImageOnScreen(meshList[GEO_BUTTON_NOT_PRESSED], Color(1, 1, 1), 15, 5, 62, 14);
			RenderTextOnScreen(meshList[GEO_TEXT], "confirm", Color(1, 1, 1), 3, 58, 13);
		}
	}

	if (tutorialStep == 3) {
		RenderImageOnScreen(meshList[GEO_NOTIF_BOX], Color(1, 1, 1), 40, 40, 40, 30);
		RenderTextOnScreen(meshList[GEO_TEXT], "Use the WASD", Color(0.6f, 0.3f, 0.3f), 3, 30, 39);
		RenderTextOnScreen(meshList[GEO_TEXT], "keys to move", Color(0.6f, 0.3f, 0.3f), 3, 30, 36);
		RenderImageOnScreen(meshList[GEO_KEYBOARD], Color(1, 1, 1), 30, 15, 40, 28);
	}

	if (tutorialStep == 4) {
		modelStack.PushMatrix();
		modelStack.Translate(16, -0.5f, 30);
		modelStack.Scale(2, 1, 2);
		RenderMesh(meshList[GEO_MARKER], false);
		modelStack.PopMatrix();

		RenderImageOnScreen(meshList[GEO_NOTIF_BOX], Color(1, 1, 1), 66, 10, 40, 10);
		RenderTextOnScreen(meshList[GEO_TEXT], "Use the WASD keys to move to the red cylinder", Color(0.6f, 0.3f, 0.3f), 3, 10, 8);
	}

	if (tutorialStep == 5) {
		RenderImageOnScreen(meshList[GEO_NOTIF_BOX], Color(1, 1, 1), 40, 10, 40, 10);
		RenderTextOnScreen(meshList[GEO_TEXT], "This watch tells you", Color(0.6f, 0.3f, 0.3f), 3, 23, 10);
		RenderTextOnScreen(meshList[GEO_TEXT], "the current in game time.", Color(0.6f, 0.3f, 0.3f), 3, 23, 7);
	}

	if (tutorialStep == 6 && !isPhoneOpen) {
		RenderImageOnScreen(meshList[GEO_NOTIF_BOX], Color(1, 1, 1), 60, 10, 35, 50);
		RenderTextOnScreen(meshList[GEO_TEXT], "This phone helps you find information", Color(0.6f, 0.3f, 0.3f), 3, 13, 50);
		RenderTextOnScreen(meshList[GEO_TEXT], "about your player. Try it out now!", Color(0.6f, 0.3f, 0.3f), 3, 13, 47);
	}

	if (tutorialStep == 7) {
		RenderImageOnScreen(meshList[GEO_NOTIF_BOX], Color(1, 1, 1), 40, 10, 40, 30);
		RenderTextOnScreen(meshList[GEO_TEXT], "Great! You're all set!", Color(0.6f, 0.3f, 0.3f), 3, 25, 30);
		RenderTextOnScreen(meshList[GEO_TEXT], "Click on the door to leave.", Color(0.6f, 0.3f, 0.3f), 3, 25, 28);
	}
...
void SceneHome::RenderPhoneUI()
{
	...
	case SceneHome::P_CHATLIST:
		break;
	case SceneHome::P_QUIT:
		RenderTextOnScreen(meshList[GEO_TEXT], "QUIT GAME", Color(0.6f, 0.3f, 0.3f), 4, 50, 40);
		RenderTextOnScreen(meshList[GEO_TEXT], "are you sure", Color(0.6f, 0.3f, 0.3f), 3, 51, 35);
		RenderTextOnScreen(meshList[GEO_TEXT], "you want to quit?", Color(0.6f, 0.3f, 0.3f), 3, 50, 32);
		RenderTextOnScreen(meshList[GEO_TEXT], "all progress will be lost.", Color(0.6f, 0.3f, 0.3f), 2, 51, 30);
		if (52 < xpos && xpos < 68 && 19 < ypos && ypos < 27 && Application::isMouseButtonPressed(0)) {
			RenderImageOnScreen(meshList[GEO_BUTTON_PRESSED], Color(1, 1, 1), 15, 7, 60, 23);
			RenderTextOnScreen(meshList[GEO_TEXT], "yes !!", Color(1, 1, 1), 4, 55, 21);
		}
		else {
			RenderImageOnScreen(meshList[GEO_BUTTON_NOT_PRESSED], Color(1, 1, 1), 15, 7, 60, 23);
			RenderTextOnScreen(meshList[GEO_TEXT], "yes !!", Color(1, 1, 1), 4, 55, 22);
		}
	...
	case SceneHome::P_HELP:
		RenderTextOnScreen(meshList[GEO_TEXT], "Move the mouse", Color(0.6f, 0.3f, 0.3f), 2.5f, 48, 40);
		RenderTextOnScreen(meshList[GEO_TEXT], "to look around.", Color(0.6f, 0.3f, 0.3f), 2.5f, 48, 37);
		RenderTextOnScreen(meshList[GEO_TEXT], "Use the WASD", Color(0.6f, 0.3f, 0.3f), 2.5f, 48, 30);
		RenderTextOnScreen(meshList[GEO_TEXT], "keys to move.", Color(0.6f, 0.3f, 0.3f), 2.5f, 48, 27);
		RenderTextOnScreen(meshList[GEO_TEXT], "Click with Mouse", Color(0.6f, 0.3f, 0.3f), 2.5f, 48, 20);
		RenderTextOnScreen(meshList[GEO_TEXT], "to interact.", Color(0.6f, 0.3f, 0.3f), 2.5f, 48, 17);
		break;
