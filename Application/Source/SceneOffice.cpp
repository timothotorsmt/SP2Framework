...
void SceneOffice::Init()
{
	...
	//computer UI
	{
		...
		meshList[GEO_NO_EMAIL_ICON] = MeshBuilder::GenerateQuad("star", Color(1, 1, 1), 1, 1, 1);
		meshList[GEO_NO_EMAIL_ICON]->textureID = LoadTGA("Image//computerUI//blank_inbox_email.tga");
...
void SceneOffice::Update(double dt)
{
	...
	if (isUsingComputer) {
		...
		case SceneOffice::C_EMAIL:
			if (showExplanations) {
				//std::cout << showThingTimer << "\n";
				showThingTimer += static_cast<float>(dt * 20);
				if (animDone == false && mouseToggle && Application::isMouseButtonPressed(0)) {
					showThingTimer = 80;
					animDone = true;
					mouseToggle = false;
				}
				if (animDone == true && mouseToggle && Application::isMouseButtonPressed(0)) {
					mouseToggle = false;
			...
			if (numUnreadEmails > 0) {
				if (showExplanations == false) {
					if (14 < xpos && xpos < 33 && 14 < ypos && ypos < 21 && mouseToggle && Application::isMouseButtonPressed(0)) {
						...
						showThingTimer = 0; animDone = false;
			...
			if (shopList[pageCounter].first.size() >= 1) {
				if (8 < xpos && xpos < 18 && 15 < ypos && ypos < 19 && mouseToggle && Application::isMouseButtonPressed(0)) { 
					...
					if (shopList[pageCounter].first[0].isScam) {
						...
						if (shopList[pageCounter].second == "HDMI CABLES") { phone->markTaskAsCompleted("Buy HDMI Cable"); NotificationTimer = 3; notif = N_TASK_DONE; }
						if (shopList[pageCounter].second == "MOBILE PHONES") { phone->markTaskAsCompleted("Buy new phone"); NotificationTimer = 3; notif = N_TASK_DONE; }
						if (shopList[pageCounter].second == "WATCHES") { phone->markTaskAsCompleted("Buy new watch"); NotificationTimer = 3; notif = N_TASK_DONE; }
					}
				}
			}
...
void SceneOffice::Reset()
{
	...
	//work computer 
	angleBetweenComputer = 0;
	isUsingComputer = false;
	for (bool x : comHoverAnimation) {
		x = false;
	}
	...
	printerTimer = 0;
	workRandomiser = 0;
	hasPrinted = false, hasScanned = false;
	tasksDone = 0;
	itemsToOrganise = 0;
	TypedCharacter = "A";
	charEnterCounter = 0;
...
void SceneOffice::RenderComputerUI()
{
	...
	case SceneOffice::C_EMAIL:
		if (numUnreadEmails == 0) {
			...
			RenderImageOnScreen(meshList[GEO_NO_EMAIL_ICON], Color(1, 1, 1), 40, 25, 40, 30);
		}
		else {
			...
			if (showExplanations) {
				...
				case 0:
					//not scam lol
					if (showThingTimer <= 28) {
						RenderImageOnScreen(meshList[GEO_SOCIALSCORE_BAR], Color(1, 1, 1), (showThingTimer), 2, 20 + (showThingTimer / 2), 39);
					}
					else {
						animDone = true;
						RenderImageOnScreen(meshList[GEO_SOCIALSCORE_BAR], Color(1, 1, 1), 28, 2, 34, 39);
						RenderTextOnScreen(meshList[GEO_TEXT], "Reliable Sender", Color(0.3f, 0.6f, 0.3f), 2.5f, 36, 40);
					}
					break;
				case 1:
					if (showThingTimer <= 30) {
						RenderImageOnScreen(meshList[GEO_SOCIALSCORE_BAR], Color(1, 1, 1), (showThingTimer), 2, 20 + (showThingTimer / 2), 39);
					}
					else {
						animDone = true;
						RenderImageOnScreen(meshList[GEO_SOCIALSCORE_BAR], Color(1, 1, 1), 30, 2, 34, 39);
						RenderTextOnScreen(meshList[GEO_TEXT], "Unreliable Sender", Color(0.3f, 0.6f, 0.3f), 2.5f, 42, 41);
					}
					break;
				case 2:
					if (showThingTimer <= 14) {
						RenderImageOnScreen(meshList[GEO_SOCIALSCORE_BAR], Color(1, 1, 1), (showThingTimer), 2, 39 + (showThingTimer / 2), 39);
					}
					else {
						RenderImageOnScreen(meshList[GEO_SOCIALSCORE_BAR], Color(1, 1, 1), 14, 2, 46, 39);
					}
					if (showThingTimer <= 21) {
						animDone = true;
						RenderImageOnScreen(meshList[GEO_SOCIALSCORE_BAR], Color(1, 1, 1), (showThingTimer), 2, 19.5f + (showThingTimer / 2), 41);
					}
					else {
					...
					break;
				case 3:
					if (showThingTimer <= 56) {
						RenderImageOnScreen(meshList[GEO_SOCIALSCORE_BAR], Color(1, 1, 1), (showThingTimer), 5, 12 + (showThingTimer / 2), 32);
					}
					else {
						animDone = true;
					...
					break;
				case 4:
					if (showThingTimer <= 20) {
						RenderImageOnScreen(meshList[GEO_SOCIALSCORE_BAR], Color(1, 1, 1), (showThingTimer), 3, 15 + (showThingTimer / 2), 30);
					}
					else {
						animDone = true;
			...
			//render buttons
			if (14 < xpos && xpos < 33 && 14 < ypos && ypos < 21 && Application::isMouseButtonPressed(0) && showExplanations == true) {
			...
			if (45 < xpos && xpos < 64 && 14 < ypos && ypos < 21 && Application::isMouseButtonPressed(0) && showExplanations == true) {
	...
	case SceneOffice::C_TASKS:
		...
		if (static_cast<int>(phone->taskList.size()) - 5 - pageCounter > 0) {
			if (!phone->taskList[static_cast<int>(phone->taskList.size()) - 5 - pageCounter].isCompleted) { RenderTextOnScreen(meshList[GEO_TEXT], "-", Color(0.6f, 0.3f, 0.3f), 3, 25, 18); RenderTextOnScreen(meshList[GEO_TEXT], phone->taskList[static_cast<int>(phone->taskList.size()) - 5 - pageCounter].taskName, Color(0.6f, 0.3f, 0.3f), 3, 30, 18); }
			else {
