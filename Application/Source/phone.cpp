...
bool Phone::findInTaskList(std::string taskToFind)
{
	//sequential search cos it's not sorted fuck
	//TODO: find a search algorithm
	for (unsigned i = 0; i < taskList.size(); i++) {
		if (taskList[i].taskName == taskToFind) {
			return true;
		}
	}
	return false;
